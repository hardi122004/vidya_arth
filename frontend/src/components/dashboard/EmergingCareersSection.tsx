...
import { EMERGING_CAREERS, type EmergingCareer } from "@/data/dashboardMockData";
import { cn } from "@/lib/utils";

const DEMAND_STYLES: Record<EmergingCareer["demand"], string> = {
...
export function EmergingCareersSection() {
  return (
    <ScrollRow title="Emerging Career Roles" subtitle="High-demand careers shaping the next decade">
      {EMERGING_CAREERS.map(({ name, icon: Icon, growth, demand }, idx) => (
        <motion.div
          key={name}
          ...
          whileHover={{ y: -4 }}
          className="glass-card group flex w-64 shrink-0 snap-start flex-col gap-4 p-5"
        >
