...
const NAV_LINKS = ["Dashboard", "Learners", "Sessions", "Community", "Analytics"];
...
export function MentorNavbar() {
  ...
  const activeLink = location.pathname === "/mentor-dashboard" ? "Dashboard" : "";
  ...
  const initial = user?.full_name?.charAt(0).toUpperCase() ?? "M";
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-border/60 bg-background/70 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 rounded-full border border-border/60 bg-surface/60 p-1 lg:flex">
          {NAV_LINKS.map((link) => (
            <button
              key={link}
              type="button"
              onClick={() => {
                if (link === "Dashboard") navigate("/mentor-dashboard");
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          <div className="relative" ref={messagesRef}>
            ...
            >
              ...
              <span className="absolute right-2.5 top-2.5 h-2 w-2 rounded-full bg-brand-blue-500 ring-2 ring-background" />
            </button>
              ...
              {messagesOpen && (
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    No new messages yet. Learner conversations will show up here.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {notificationsOpen && (
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-muted-foreground">
                    You're all caught up! New learner requests and session updates will show up here.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <AnimatePresence>
              {profileOpen && (
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => handleComingSoon("Personal Details")}
                  ...
                  <button
                    type="button"
                    onClick={() => handleComingSoon("Professional Details")}
                    className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-foreground transition hover:bg-accent"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => handleComingSoon("Expertise & Domains")}
                    className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-medium text-foreground transition hover:bg-accent"
                  >
